...
INPUT_PLUGIN_TABLE* Plugin::getInputPlugin() const
{
...
BOOL Media::open(PluginPtr plugin, LPCTSTR fileName)
{
	MY_TRACE(_T("Media::open(%s)\n"), fileName);

	m_plugin = plugin;
	INPUT_PLUGIN_TABLE* ip = m_plugin->getInputPlugin();

	// メディアを開く。
	m_fileName = fileName;
	MY_TRACE_TSTR((LPCTSTR)fileName);

	m_inputHandle = ip->func_open(m_fileName);
...
void* Media::readVideo(int32_t frame, int32_t* bufferSize)
{
	...
	INPUT_PLUGIN_TABLE* ip = getPlugin()->getInputPlugin();
